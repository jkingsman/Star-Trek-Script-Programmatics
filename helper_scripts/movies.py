...
def getLinesFromStringBlock(block):
    ...
    for j in range(int(len(split_scene_text) / 2)):
        ...
        if "[" in character_block:
            ...
            if modifier in ["OC", "CO"]:
                line['modifier'] = "voiceover"
            elif modifier in ["on monitor", "on viewscreen", "On viewscreen", "on screen", "on PADD"]:
...
directory = 'www.chakoteya.net/movies'
for entry in os.scandir(directory):
    if not entry.name.startswith('movie') or not entry.name.endswith('.html'):
        continue

    episode = {}
    movie_num = int(entry.name[5:-5])
    ...
    episode['schedule'] = {"number": movie_num}
    ...
    with open("json_transcripts/movies/movie%s - %s.json" % (str(movie_num).zfill(2), safe_filename(episode['title'])), 'w') as outfile:
